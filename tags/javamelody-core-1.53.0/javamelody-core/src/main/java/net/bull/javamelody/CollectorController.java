...
class CollectorController {
...

	{
			return new ArrayList<List<ThreadInformations>>(
					collectorServer	final MonitoringController monitoringController = new MonitoringController(
	...
	private void doMultiHtmlProxy(HttpServletRequest req, HttpServletResponse resp,
		...
		I18N.writelnTo(
				"<a href='javascript:history.back()'><img src='?resource=action_back.png' alt='#Retour#'/> #Retour class='chapterTitle'><img;&nbsp;&nbsp;",
				writer);
	...
	private Serializable createSerializable(HttpServletRequest httpRequest, String application,
		...
		if (THREADS_PART.equalsIgnoreCase(part)) {
		...
		} else if (COUNTER_SUMMARY_PAPPLICATIONS_PART.equalsIgnoreCase(part)) {
			...
			return new HashMap<String, Throwable>(applicationsCOUNTER_SUMMARY_PER_CLASS_PART.equalsIgnoreCase(part)) {
